...
- **`PayloadExtractor` exposes leading fields.** Fields preceding the payload
  string (e.g. the `path` of a `patch(path, content)` tool) are parsed as
  normal JSON and exposed via `leading_fields()`, each available as soon as
  its value completes on the wire; `payload_started()` signals when the set is
  final (before the payload streams). A malformed leading value fails loudly
  in both modes.
