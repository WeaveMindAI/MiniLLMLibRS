...
- The crate forced a native-TLS (OpenSSL) dependency on every consumer: its
  `reqwest` used default features. A consumer building in a slim container
  (no OpenSSL dev headers) failed to compile. It now uses rustls, which needs
